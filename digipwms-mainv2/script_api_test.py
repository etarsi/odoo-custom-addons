...
headers["X-API-KEY"] = "SebiGus_BACKUP_REAL"
false=False
true=True
null=None
...
def actualizo_bultos(p,b):
    newc={}
    newc["Codigo"]= p
    newc["ServicioDeEnvioTipo"]= b
    print('%s/v1/Pedidos/%s' % (url,p))
    print(newc)
    respPut = requests.put('%s/v1/Pedidos/%s' % (url,p), headers=headers, json=newc)
    print(respPut)
    print(respPut.content)

actualizo_bultos('D12',16)
