from yatl import  DIV, TABLE, TR, TD, TH, A, SPAN, I, THEAD, P, TAG, TBODY, SCRIPT
...
"""
    ...
    def script(self):
        js = (
            ...
            "            processing: true, "
            "            serverSide: true, "
            "            lengthMenu: [  [10, 15, 20, 9999999999], [10, 15, 20, 'All']  ], "
            "            pageLength: %s, "
            '            ajax: "%s", '
        ...
        for btn in self.buttons:
            btn_tmp=A(btn.label,_class=btn.clase,_href=btn.href,_onclick=btn.onclick)
            js += (
                "{"
                "    data: null,"
                "    render: function ( data, type, row ) {"
                "      var %s_url='%s'; "
                '      %s_url = %s_url.replace("record_id", row.DT_RowId); '
                '      return %s_url  '
                "    }, "
                "    orderable: false, "
                "}, "
                % (btn.name,btn_tmp,btn.name,btn.name,btn.name)
            )
       ...
       #)
        ...
        for index, field in enumerate(self.fields):
            if not field.orderable:
                js += '{"orderable": false, "targets": %s},' % index
    ...
    def table(self):
        ...
        if self.create_url and self.create_url != "":
            ...
            _span.append(I(_class="fa fa-plus"))
            _a.append(_span)
            _a.append(SPAN("New"))
            _html.append(_a)
        ...
        _table = TABLE(
            ...
            _class="table table-striped table-head-fixed dataTable dtr-inline'",
...
class DataTablesButton:
    def __init__(
        ...
        """
        a dataholder class holding all the info we need on a field button
