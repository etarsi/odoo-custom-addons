        ...
        for att in self:
            ...
            if att.check_in and att.check_out and att.employee_id:
                ...
                if att.employee_id.employee_type == 'eventual':
                    ...
                    att.event_hours_employee()

    def event_hours_employee(self):
        for att in self:
            if att.employee_id.work_schedule_id:
                start_t = self._float_to_time(att.employee_id.work_schedule_id.hour_start + 3)
                end_t   = self._float_to_time(att.employee_id.work_schedule_id.hour_end + 3)
                ...
                if dow == 5:  # Sábado
                    start_t = self._float_to_time(att.employee_id.work_schedule_id.hour_start + 4) #comienzan 1 hora más tarde
                    start_dt = datetime.combine(day, start_t)
