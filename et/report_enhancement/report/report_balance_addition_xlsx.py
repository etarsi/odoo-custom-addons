...
class ReportBalanceAdditionXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, wizards):
        wiz = wizards[:1]
        lines, totals = wiz._get_lines()  # ahora devuelve saldos firmados estilo OCA
        ...
        # (Opcional) anchos prolijos
        ws.set_column(0, 0, 14)  # código
        ws.set_column(1, 1, 45)  # cuenta
        ws.set_column(2, 6, 18)  # importes

        for l in lines:
            ws.write(row, 0, l["account_code"])
            ws.write(row, 1, l["account_name"])
            ws.write_number(row, 2, l["initial_balance"], money)
            ws.write_number(row, 3, l["debit"], money)
            ws.write_number(row, 4, l["credit"], money)
            ws.write_number(row, 5, l["period_balance"], money)
            ws.write_number(row, 6, l["ending_balance"], money)
            row += 1

        # Totales
        ws.write(row, 0, "TOTALES", bold)
        ws.write_number(row, 2, totals["initial_balance"], money_bold)
        ws.write_number(row, 3, totals["debit"], money_bold)
        ws.write_number(row, 4, totals["credit"], money_bold)
        ws.write_number(row, 5, totals["period_balance"], money_bold)
        ws.write_number(row, 6, totals["ending_balance"], money_bold)
