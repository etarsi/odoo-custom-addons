...
class ImportContainerExcelTaskWizard(models.TransientModel):
    _name = 'import.container.excel.task.wizard'
    _description = 'Wizard: Importar Contenedores desde Excel en Tareas'

    wms_transfer_id = fields.Many2one('wms.transfer', string='Transferencia', required=True)
    ...
    def import_data(self):
        ...
        for cont in container_rows:
            ...
            vals_wms_task = {
                ...
                'date_start': self.fecha_llegada if self.fecha_llegada else fields.Date.today(),
                'dispatch': self.nro_despacho if self.nro_despacho else False,
                'container': cont_code,
            ...
            created_wms_task_ids.append(wms_task.id)
        ...
        return {
            ...
            'res_model': 'wms.task',
            'view_mode': 'tree,form',
            'domain': [('id', 'in', created_wms_task_ids)],
            'target': 'current',
            'name': _('Contenedores Importados en Tareas'),
        }
