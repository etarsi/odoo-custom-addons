...
class WMSTaskController(http.Controller):
    ...
    def remito_auto_merged(self, task_id, **kwargs):
    ...
    def remito_a(self, task_id, **kwargs):
        task = request.env['wms.task'].browse(task_id)
        if not task.exists():
            return request.not_found()
        ...
        filename = 'REM_%s_A.pdf' % ((task.name or str(task_id)).replace('/', '-'))
        return request.make_response(
            pdf,
            headers=[
                ('Content-Type', 'application/pdf'),
    ...
    def remito_b(self, task_id, **kwargs):
        task = request.env['wms.task'].browse(task_id)
        if not task.exists():
            return request.not_found()
        ...
        company_id = request.env['res.company'].browse(1) if tipo == 'TIPO 3' else task.company_id
        pdf = task._build_remito_pdf(task, proportion, company_id, 'b')

        filename = 'REM_%s_B.pdf' % ((task.name or str(task_id)).replace('/', '-'))
        return request.make_response(
            pdf,
            headers=[
                ('Content-Type', 'application/pdf'),
