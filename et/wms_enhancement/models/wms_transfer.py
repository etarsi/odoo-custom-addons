...
class WMSTransfer(models.Model):
    ...
    partner_address_id = fields.Many2one(string="Dirección de Entrega")
    operation_type = fields.Selection(string="Tipo de Operación", selection=[
        ('incoming', 'Ingreso'),
        ('outgoing', 'Entrega')
    ])
    sale_type = fields.Char(string="TIPO")
    sale_id = fields.Many2one(string="Pedido de Venta")
    purchase_id = fields.Many2one(string="Pedido de Compra")
    invoice_ids = fields.One2many(string="Facturas")
