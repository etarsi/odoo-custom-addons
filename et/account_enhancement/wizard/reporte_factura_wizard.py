            ...
            if facturas_lines:
                for line in facturas_lines:
                    if line.quantity==0 and line.price_unit==0:
                        pass;
                    uxb_id = line.sale_line_ids[0].product_packaging_id if line.sale_line_ids else ''
                    bultos = ''
                    if uxb_id:
                        bultos = uxb_id.qty/line.quantity
                    ...
                    worksheet.write(row, 11, uxb_id.name, formato_celdas_izquierda)
