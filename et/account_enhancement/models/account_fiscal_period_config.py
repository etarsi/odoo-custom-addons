...
class AccountFiscalPeriodConfig(models.Model):
    ...
    def action_generate_management_closure_4_5(self):
        ...
        account_proveedor_existing = self.env['account.account'].search([('company_id', '=', self.company_id.id), ('code', '=like', '4%')])
    ...
    def action_generate_opening_entries(self):
        ...
        return {
            "type": "ir.actions.act_window",
            "name": _("Asiento Contable de Apertura de Periodo (1-2-3) - %s") % self.company_id.display_name,
            "res_model": "account.move",
            "view_mode": "form",
            "res_id": created_moves.id,
            "target": "current",
